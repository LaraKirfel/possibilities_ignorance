...
## Create Long Dataframe + avg rating
...
## Create Long Dataframe with all ratings
...
## Read in Ratings from Study 1

```{r}
df.exp1rating <- read.csv("df.exp1_actions.csv")
...
figL <- density_base_i +
      ## original modal distance
         geom_segment(data=data.frame(), ##otherwise you draw this segment for every row of data...
          ...
          geom_vline(xintercept=o.c1h.xmax,alpha=.5,linetype=3,color="purple4", size=1.5) +
        ...
        ## shifted modal distance
          ...
          geom_vline(xintercept=s.c1h.xmax,alpha=1,linetype=3,color="orange3", size=1.5) +
...
#table(evSum$action[evSum$context==contextChoice])
actualAction12 <- "attempt to search for a water source"
  ...
  mutate(abnormality=100-response) %>%
    ggplot(aes(x = abnormality)) +
      ...
      geom_label(data = df.exp2rating_k %>% filter(scenario == contextChoice & action==actualAction2) %>%
                                     ...
                                     mutate(abnormality=100-mean,
                                            action=factor(action),
                                            action=factor(c("search water source")[action]) 
                                            ), 
                 aes(x = abnormality, y = -0.0005, 
                     label = action), 
                 color="purple4",
...
figR <- density_base_i2 +
      ## original modal distance
         geom_segment(data=data.frame(), ##otherwise you draw this segment for every row of data...
                  ...
                  arrow = arrow(length = unit(0.40, "cm")),
                  color="purple4", alpha=.75, linetype=1) +
          geom_vline(xintercept=o.c1h.xmax2,alpha=.5,linetype=3,color="purple4", size = 1.5) +
        ...
        ## shifted modal distance
          ...
          arrow = arrow(length = unit(0.45, "cm")),
          color="orange3", alpha=.35, linetype=1) +
          geom_vline(xintercept=s.c1h.xmax2,alpha=1,linetype=3,color="orange3", size = 1.5) +
...
## Add both FigL and FigR together
...
```
## Merge modal distance plot with point range plot
...
ggsave("../figures/plots/figModalDistanceMean.pdf", plot = modalmeans_plot2, 
...
## Create Dataset for Chi Square Plot
...
# Save as PDF
ggsave(plot = FigChiSq, "../figures/plots/figChiSq.pdf", width = 8, height = 6)
